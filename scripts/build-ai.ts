#!/usr/bin/env node
// Port of buid_ai.py to Node.js for Windows compatibility.
// Reads .ai/PLAN.MD, patches sections 12 and Appendix D, then splits
// into .ai/skills/*.md, .ai/templates/*.md, .ai/README.md etc.

import fs from "node:fs";
import path from "node:path";

const ROOT = path.resolve(".");
const PLAN_PATH = path.join(ROOT, ".ai", "PLAN.MD");
const OUT = path.join(ROOT, ".ai");

// ---------------------------------------------------------------------------
// 1. Load and patch the master plan
// ---------------------------------------------------------------------------
let plan = fs.readFileSync(PLAN_PATH, "utf-8").replace(/\r\n/g, "\n");

const part12 = `## 12. Schedule and deadline
...
// Find and replace section 12
const marker12start = "## 12. Suggested timeline";
const marker12end = "## 13. Final checklist before you submit";
const a = plan.indexOf(marker12start);
const b = plan.indexOf(marker12end);
if (a === -1 || b === -1) {
  console.error("Could not find section 12 or 13 in PLAN.MD");
  process.exit(1);
}
plan = plan.slice(0, a) + part12 + plan.slice(b);

// Append Appendix D
plan = plan.trimEnd() + "\n" + appendixD;

fs.writeFileSync(PLAN_PATH, plan, "utf-8");
console.log("Patched PLAN.MD");
...
function getSection(prefix: string, level: number): string {
  const pat = "#".repeat(level) + " " + prefix;
  let start: number | null = null;
  let inFence = false;
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i];
    if (l.startsWith("```")) inFence = !inFence;
    if (!inFence && l.startsWith(pat)) {
      start = i;
      break;
    }
  }
  if (start === null) throw new Error(`Section not found: ${prefix}`);
  ...
  while (block.length > 0 && ["", "---"].includes(block[block.length - 1].trim())) block.pop();
...
// ---------------------------------------------------------------------------
// 3. Ensure output directories exist (non-destructive)
// ---------------------------------------------------------------------------
...
function w(relPath: string, text: string) {
...
function skill(
  ...
  const parts = sections.map(([p, lv]) => getSection(p, lv));
  const body = parts.join("\n\n");
  const fm = `---\nname: ${name}\nload_when: ${loadWhen}\ndepends_on: [${depends}]\nrelated: [${related}]\ncode: ${code}\n---\n\n`;
  const title = "# " + name.replace(/-/g, " ").replace(/^\w/, (c) => c.toUpperCase()) + "\n\n";
  const d = "\n\n## Done when\n" + done.map((x) => "- " + x).join("\n");
  const txt = fm + title + body + (extra ? "\n\n" + extra.trim() : "") + d + FOOT;
  w("skills/" + fname, txt);
  console.log("  Written: skills/" + fname);
}

// ---------------------------------------------------------------------------
// 4. Sliced skills
// ---------------------------------------------------------------------------
console.log("Writing skill files...");
...
w("SCHEDULE.md", `# Schedule (load when planning a step or reporting progress)
...
interface Req { grp: string; rows: [string, string, string][] }
const reqs: Req[] = [
 {grp:"AUTH", rows:[
...
w("templates/checkpoint-report.md", `# Checkpoint report: <step name>
...
w("templates/decision-entry.md", `## D<n>. <title>
Decision:
Options considered:
Reason:
Consequence in code:
...
console.log("Done. .ai/ index, schedule, requirements and all skill files written.");
